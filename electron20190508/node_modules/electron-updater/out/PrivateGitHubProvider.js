...
class PrivateGitHubProvider extends _GitHubProvider().BaseGitHubProvider {
  ...
  getLatestVersionInfo(cancellationToken) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      let baseUrl = _this2.basePath;
      const allowPrerelease = _this2.updater.allowPrerelease;

      if (!allowPrerelease) {
        baseUrl = `${baseUrl}/latest`;
      }

      const url = (0, _main().newUrlFromBase)(`${baseUrl}`, _this2.baseUrl);

      try {
        let version = JSON.parse((yield _this2.httpRequest(url, _this2.configureHeaders("application/vnd.github.v3+json"), cancellationToken)));
