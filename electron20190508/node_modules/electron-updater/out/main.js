...
function newUrlFromBase(pathname, baseUrl, addRandomQueryToAvoidCaching = false) {
  const result = new (_url().URL)(pathname, baseUrl);
  const hasSearch = result.search != null && result.search.length !== 0; // search is not propagated (search is an empty string if not specified)
