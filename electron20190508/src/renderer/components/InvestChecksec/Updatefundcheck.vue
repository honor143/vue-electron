<template>
    <div class="bond-info padtop5 padright5">
        <div class="sec-header check-header">
            <div>
                <h3>账户修改审批<Badge :count="waitCheck"></Badge></h3>
            </div>
            <div class="align-center">
                <Checkbox v-model="ischecked" @on-change="selectCheck">显示已审批</Checkbox>
                &nbsp;<label for="">投标截止日期</label>
                <!-- <DatePicker size="small" :value="lIssueEndDate" format="yyyy-MM-dd" type="date" placeholder="选择日期" style="width:120px" @on-change="selectBidDate"></DatePicker> -->
                ...
                <!-- <Input :value="vcSname" size="small" placeholder="债券简称" style="width: 120px" @on-change="selectBondShortName"/> -->
                <!-- <Input clearable v-model="vcSname" size="small" placeholder="债券简称" style="width: 120px" @on-change="selectBidOrderCheck"/> -->
        ...
        <Row :gutter="5" class="bottom-height">
            <Col span="24" class="height100">
                <div class="sec-header">
                    <h3>账户修改审批明细</h3>
                    <Checkbox v-model="ischeckeddetail" @on-change="selectDetail" style="margin-left:58px;">显示已审批</Checkbox>
                    <div class="right-wrapper" style="top:3px;">
                        <Button size="small" type="primary" @click="reject"  v-if="userButtonRoles.submitBondChange_550_3">拒绝</Button>
                        <Button size="small" type="primary" @click="agree"  v-if="userButtonRoles.submitBondChange_550_3">同意</Button>
                    </div>
                </div>
...
export default {
    ...
    data(){
        return{
            ...
            userButtonRoles:{
                ...
                submitBondChange_550_3:false//审批
            },
            time:{
                D:'',
                h:'',
            ...
            bidorderdetail:[
                ...
                {
                    title: '原账户',
                    key: 'vcFundNameBefore',
                    minWidth:100
                },
                {
                    title: '原组合',
                    key: 'vcCombiNameBefore',
                    minWidth:100
                },
                {
                    title: '原市场',
                    key: 'vcMarketNameBefore',
                    minWidth:100
                },
                {
                    title: '原经理',
                    key: 'vcDisplaynameBefore',
                    minWidth:100
                },
                {
                    title: '修改后账户',
                ...
                {
                    title: '修改后组合',
                    key: 'vcCombiNameAfter',
                    minWidth:100
                },
                {
                    title: '修改后市场',
                    key: 'vcMarketNameAfter',
                    minWidth:100
                },
                 {
                    title: '修改后经理',
                    key: 'vcDisplaynameAfter',
                    minWidth:100
                },
    ...
    methods:{
        ...
        selectBidOrderUncheckNum(){
            this.$httpGet(`/bondChange/selectApprovalCount?lBidDate=${this.lIssueEndDate}&vcSname=${this.vcSname}`)
                .then((res) => {    
                    if(res.status===200){
                        this.waitCheck = res.data == 0 ? null : res.data
                    }                
                })
        },
        //查询投标指令审批
        selectBidOrderCheck(){
            ...
            // &vcSname=${this.vcSname}lIssueEndDate=${this.lIssueEndDate}&
            this.$httpGet(`/bondChange/selectBondChangeApprovals?isShow=${this.orderStatus}&lBidDate=${this.lIssueEndDate}&vcSname=${this.vcSname}&pageNum=${this.pageNumCheck}&pageSize=${this.pageSizeCheck}`)
                .then((res) => {            
        ...
        selectBidOrderDetail(){
            ...
            // lIssueCnt=${this.lIssueCnt}&fundType=${this.fundType}
            this.$httpGet(`/bondChange/selectBondChangeApprovalDetails?isShow=${this.detailStatus}&lIssueCnt=${this.lIssueCnt}&pageNum=${this.pageNumDetail}&pageSize=${this.pageSizeDetail}&vcCode=${this.vcCode}`)
                .then((res) => {            
                    if(res.status===200 && res.data){
       ...
       /*  refreshSum(){
        ...
        ok(name){
            if(this.modalTip==="审批"){//批量审批投标指令
                let bool=this.adviceObj.title==="审批拒绝,意见如下"?0:1;
                ...
                this.$httpPost(`/bondChange/submitBondChange?`,{taskIds:this.selectArr,vcRemarks:this.adviceObj.text,status:bool})
